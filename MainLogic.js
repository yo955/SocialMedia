  ...
  axios
    ...
    .then(function (response) {
      ...
      for (let post of posts) {
        ...
        if (isMyPost) {
          EditButonContent = `
               <button class="btn btn-primary" onclick="Edit_Btn_Clicked('${encodeURIComponent(
                 JSON.stringify(post)
               )}')">
                  Edit
               </button>
               <button class="btn btn-danger" onclick="Delete_Btn_Clicked('${encodeURIComponent(
                 JSON.stringify(post)
               )}')">
               Delete
               </button>
...
const handleRegisterbtn = () => {
  ...
  const profile_image = document.getElementById("register-image-input")
    .files[0]; // صورة الملف الشخصي
...
function handleAddPost() {
  ...
  const textArea_post_input = document.getElementById(
    "textarea-post-input"
  ).value;
